...
let SocialObject = require('./SocialObject.js');
let std_msgs = _finder('std_msgs');

//-----------------------------------------------------------

class SocialObjectsInFrame {
  constructor(initObj={}) {
    if (initObj === null) {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.objects.forEach((val) => {
      bufferOffset = SocialObject.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    let len;
    let data = new SocialObjectsInFrame(null);
    ...
    for (let i = 0; i < len; ++i) {
      data.objects[i] = SocialObject.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    ...
    object.objects.forEach((val) => {
      length += SocialObject.getMessageSize(val);
    });
    return length + 4;
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new SocialObjectsInFrame(null);
    ...
    if (msg.objects !== undefined) {
      ...
      for (let i = 0; i < resolved.objects.length; ++i) {
        resolved.objects[i] = SocialObject.Resolve(msg.objects[i]);
      }
    }
    else {
      resolved.objects = []
...
module.exports = SocialObjectsInFrame;
